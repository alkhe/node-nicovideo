...
function Nicovideo(options) {
    ...
    this.options.url.ivt = this.options.url.ivt || 'http://i.nicovideo.jp/';
    // this.options.url.www = this.options.url.www || 'http://www.nicovideo.jp/';
...
Nicovideo.prototype.getthumbinfo = function(id, format, cb) {
    ...
    var u = geturl(this.options.url.ivt, 'v3', 'video.array', '?v=' + id);
...
Nicovideo.prototype.login = function(mail, password, cb) {
    exec('python ./node_modules/node-nicovideo/lib/auth.py ' + mail + ' ' + password, function(err, stdout, stderr) {
